...
@apps_blueprint.route("/apps/<key>/thermostats/<identifier>/", methods=["GET", "POST"])
...
@apps_blueprint.route("/apps/<key>/thermostats/<identifier>/resume", methods=["POST"])
def resume(key, identifier):
    app = getApp(key)
    app.resume(identifier=identifier)
    return redirect(
        url_for("apps_blueprint.thermostat", key=key, identifier=identifier)
    )


@apps_blueprint.route(
    "/apps/<key>/thermostats/<identifier>/set_hvac_mode", methods=["POST"]
)
...
@apps_blueprint.route(
    "/apps/<key>/thermostats/<identifier>/send_message", methods=["POST"]
)
...
@apps_blueprint.route(
    "/apps/<key>/thermostats/<identifier>/set_climate_hold", methods=["POST"]
)
...
@apps_blueprint.route(
    "/apps/<key>/thermostats/<identifier>/set_temperature_hold", methods=["POST"]
)
